...
- `needs_literature_screening`：没有可承接的材料证据或目录记录；页面说明目录覆盖缺口，并保留可推进的材料路线、候选核验项和文献筛选入口。
- `catalogue_guided_start`：用户尚未给出指标、但上文已有可理解的部位或场景；服务先从目录中给出少量有来源记录的起步候选，并以自然语言逐步收集工况和关注点。起步候选不是硬筛选结论。
- `needs_screening_criteria`：用户提出开放式选材，且没有足以识别起步候选的场景或工况；服务用简短问题收集用途，不要求用户预先知道商品牌号。

导电液体路径在数值窗口归零时返回 `fluid_no_matching_evidence`：保留温度、电学、黏度和证据质量的完整筛选漏斗，明确显示在哪一步归零，不擅自放宽窗口或推荐替代配方。通用固体材料路径在目录证据不足时可以展示场景化的材料路线和待验证项，但不得把它们写成已入库候选、商品牌号、性质事实或工程放行。结果状态仅记录在最终 manifest 的 `data_status.outcome` 中；前端保持既有文本流、`progress` 和 `result` 事件，不需要增加新事件类型。
...
通用目录既是材料索引，也是选材初筛：当用户明确材料名称、牌号或标准、但未给出性能条件时，进入 `catalogue_index`，直接核验身份、产品状态、已收录性质和来源，不要求补充人为筛选条件，也不输出排序或通过结论。开放式选材按已明确的筛选维度处理：0 项仍不执行硬筛选；若上文已说明部位或场景，则进入 `catalogue_guided_start`，仅展示少量有来源记录的起步候选并继续追问；若连场景也不足，则只收集用途。1 项输出单条件证据地图，不作综合优先级；2 项执行交叉过滤；3 项及以上执行严格多条件证据筛选。维度包括材料锚点/体系、服役温度、应用、环境、制造约束，以及每一项独立的性能目标（两个不同性质阈值计作两个维度）。自然语言中明确写出的“指标 + 比较符 + 数值 + 单位”会转为硬阈值，例如“导热率≥100 W/(m·K)、屈服强度≥600 MPa”；“指标 + 下限-上限 + 单位”会展开为两条上下界，例如“屈服强度600-800 MPa”。裸数字或没有单位的定性表述不会被猜成阈值。每个 manifest 的 `screening.strategy` 记录实际模式，`screening.next_action` 明确为 `continue_guided_discovery`、`await_user_criteria` 或 `return_catalogue_evidence`；上游在前两者时继续追问，不把起步候选写成硬筛选结论。
