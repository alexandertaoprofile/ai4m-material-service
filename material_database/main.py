...
def roles():
    ...
    return {
        profile: {
            "name": ROLE_NAME,
            "profile": profile,
            "goal": "形成已有材料的可追溯选型证据链，核验材料是否满足需求指标。",
            "constraints": "在既定材料库或标准牌号范围内，基于需求指标、材料身份和使用工况开展检索、匹配与横向比对。",
            ...
            "addresses": ["src.team_config.MaterialMature", ROLE_NAME],
            ...
            "routing": {
                ...
                "priority": 2,
                "match_when": "需要在既定材料库或标准牌号范围内，根据需求指标检索、匹配并横向比对已有材料时。",
                "include_keywords": ["已有材料", "材料牌号", "材料筛选与计算", "材料筛选", "材料选型", "候选材料", "性质对比", "导电润滑", "导电润滑油", "润滑介质", "旋转黏度", "旋转粘度", "电导率", "电阻率", "商用耗材", "丝材", "FDM", "FFF", "PLA", "PETG", "ASA", "ABS", "PC", "PA", "PEEK", "商品名", "牌号", "标准号", "UNS", "AMS", "MIL", "ASTM", "GB/T", "Inconel", "TIMETAL", "316L", "高温合金牌号", "高熵合金牌号", "HEA 牌号", "MPEA 牌号", "合金标准号", "商品合金牌号"],
                "exclude_keywords": ["严格化学式", "元素比例", "原子百分比", "成分优化", "配比优化", "合金配比", "合金成分", "成分空间", "组分设计"],
                "route_before": [],
                "workflow_hint": "本服务只有成熟材料目录核验与导电润滑介质初筛两条工作流。自定义合金的元素比例、成分空间和配方优化应直接调用合金成分优化服务。",
                "input_contract": {
                    "required_any": ["material_queries/材料名称", "厂家或牌号", "标准号", "upstream_evidence（性质、工况、来源）", "导电润滑应用 + 电导率/电阻率或动态黏度数值条件"],
