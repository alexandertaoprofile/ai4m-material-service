...
class MatterGenDiscovery(Action):
    ...
    async def run(self, instruction: str, *args):
        ...
        progress_task = asyncio.create_task(
            stream_discovery_progress(websocket, results_root / constraints.taskid, constraints.taskid, step_id="NEW_MATERIAL_DISCOVERY")
        )
        try:
...
class XIMUAlpha_MNS(Role):
    ...
    profile: str = (
        ...
        "validation_targets、max_candidates；也可直接使用自然语言或上文上下文。后者会确定性提取元素体系、显式 E_hull 阈值和验证关注点，"
        "并在前端标注自动提取结果；若元素体系无法可靠确定，则要求补充而不盲目生成。"
