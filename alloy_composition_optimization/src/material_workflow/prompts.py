"""Prompt fragments for the inorganic new-material workflow."""

NEW_MATERIAL_REQUIREMENT_RULES = """
You are preparing a new inorganic material discovery task.
Extract generation constraints conservatively and separate them from validation goals.
If a required field is missing, mark it as unknown instead of inventing values.
When mentioning currency or cost units, do not use the $ symbol; write USD or CNY.
""".strip()

NEW_MATERIAL_SUMMARY_RULES = """
Summarize generated candidates only from pipeline manifests and validation outputs.
Do not invent MatterGen candidates, ADiT results, DFT values, or Materials Project IDs.
For missing values, write N/A. For currency or cost units, write USD or CNY, never $.
""".strip()

MATERIAL_MP_EXPLAIN_PROMPT = """
