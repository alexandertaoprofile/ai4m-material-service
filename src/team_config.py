...
API_KEY = os.getenv("api_key", "ximu-llm-api-key")
XIMU_MNS_FLOW2_RANKING_BRIEF_PROMPT = """
                你是 XIMUAlpha_MNS 平台中的流程2候选排序播报模块。
        你的任务是：基于初筛数据 {parameters}，结合上一阶段上下文 {previous_flow_context}，生成逻辑严密的候选排序报告。

        ---
        【核心输出要求】
        请根据 {previous_flow_context} 的内容，选择对应的【决策综述】风格：

        - **场景 A（有前置信息）**：输出一段 150 字以内的“需求映射分析”。请直接说明为何本轮锁定的材料种类（如含氧有机物等）能满足前文提到的具体工艺挑战（如 mSAP、Df 损耗等）。

        - **场景 B（无前序信息）**：直接说明我们将按 [性能50%, 寿命20%, 成本15%, 工艺10%, 环保5%] 的标准专家权重进行评分。
        *注意：禁止分条目，禁止自我重复，禁止使用“【流程衔接】”等标签作为开头。*

        ---
        【固定展示结构】
        ### 专家筛选综述
        > (此处仅输出上述逻辑生成的分析段落，确保语气专业且唯一)
        > 结合上下文进行详细论述

        ### 候选材料排序表
        | 排名 | 候选化学式 | 来源条目 | 匹配标签 | 综合评分 | 状态说明 |
        | :--- | :--- | :--- | :--- | :--- | :--- |
        (仅根据 {parameters} 填表，第一名状态为“进入计算”，其余为“候选保留”)
        - 候选化学式来自parameters的formulas
        - 来源条目来自parameters的title
        - 匹配标签来自parameters的keywords
        以上三个内容严格选自{parameters}，不可自己生成
        
        

        ### 执行决策
        - 排名第一的候选材料已正式锁定，即刻进入下一步深度计算模拟。
...
class Coding(Action):
    ...
    XIMU_MNS_FLOW2_RANKING_BRIEF_PROMPT: str = """
        ...
        *注意：禁止分条目，禁止自我重复，禁止使用“【流程衔接】”等标签作为开头。*
        ...
        - 不要输出代码块标记。
         ...
         """
    XIMU_MNS_MATERIAL_INTEGRATED_SCREENING_PROMPT: str = """
            你是 XIMUAlpha_MNS 平台的材料仿真结果整合模块。
            你的任务是：整合 Materials Project、ADiT 稳定性评估及 MACE 模拟的所有计算结果，生成一份高密度的“材料全维度筛选报告”。
            ...
            ---
            多阶段综合计算数据（包含 MP/ADiT/MACE 等所有参数）：
            {parameters}

            ---
            【逻辑分层与内容整合规则】
            1. **深度去重与特征融合**：
               - 严禁对同一 material_id 进行重复条目展示；
               - 必须将跨阶段（如 MP、ADiT、MACE）的散点数据，按 ID 维度聚合成唯一的“数字孪生”特征行。

            2. **章节逻辑定义**：
               - **专家评审综述**：
                 - 动态提取 {query} 或 {summary} 中的核心性能诉求作为评价基准；
                 - 对比计算得出的实际数值，定量评价当前候选体系对原始设想的支撑程度。

               - **全维度候选对比总表**：
                 - 表格必须包含 ID 字段，并横向集成 {parameters} 中所有已出现的关键物理判据与工程门槛指标；
                 - 优先展示具有“Gate/Pass”属性的决策字段，确保筛选漏斗的透明度。

            3. **表格动态列名约束**：
               - 表格列名应根据 {parameters} 实际含有的字段动态生成，但格式必须锁定为：
                 | 指标名称 | 物理含义 | 原始取值 | 工程判据 | 结论 |

            ---
            【内容表达规则】
            - 必须保持工程计算视角，严禁科普或教学性表述 [cite: 3]；
            - 禁止使用“本案例 / 本系统 / 我们”等指代 ；
            - 语气必须保持工程记录式、克制、客观 [cite: 12, 19]；
            - 禁止总结性结论或展望性描述 [cite: 11, 19]；
            - 表格中禁止出现任何 LaTeX 语法（包括 $ 、^ 、_ 等） [cite: 6]；
            - 若某字段缺失或未计算，表格取值写“待计算”，不得写“未提供” [cite: 15, 18, 34]。
    ...
    async def run(self, instruction: str, *args):
        ...
        # =========================
        # await websocket.send_text(f"这是上一步的输出、本步接入{instruction}\n")
        # await 
        norm = _normalize_user_text(instruction)
        # await websocket.send_text(f"这是norm{norm}\n")
        previous_flow_context = _extract_previous_flow_context(instruction, file_metadata)
        # await websocket.send_text(f"这是flow{previous_flow_context}\n")
        route, content = _parse_route(norm)
        # await websocket.send_text(f"这是route{route}\n")
        ...
        if route != "llm":
            ...
            # await websocket.send_text(f"这是formulas{formulas}\n")
            ...
            if not formulas:
                if _is_smalltalk(norm):
                    ...
                    print("【提示】未识别到可用化学式，且未能从需求中生成候选材料。\n")
                    # await websocket.send_text(
            ...
            if formulas:
                ...
                def _read_mace_summary(repo_root: str, root_path: str, taskid_: str, formula_: str, pipeline_: str) -> dict:
                    import os, glob, json
                    abs_case_root = os.path.abspath(os.path.join(repo_root, root_path))
                    results_dir = os.path.join(abs_case_root, "results")
                    taskid_s = str(taskid_).replace("/", "_")
                ...
                def _patch_summary_md_header(repo_root: str, root_path: str, taskid_: str, formula_: str, pipeline_: str, header_line: str):
                    import os, glob
                    abs_case_root = os.path.abspath(os.path.join(repo_root, root_path))
                    results_dir = os.path.join(abs_case_root, "results")
                    taskid_s = str(taskid_).replace("/", "_")
                ...
                await websocket.send_text("\n【信息】MACE 性质计算阶段已完成。\n")
                return
        parameters="/home/ubuntu/Zhuolun_project/MNS_Tuutorial/ALPHA-MNS-main/material-screen-calc/ai4m_tqm/src/material_pipeline/pcb_material_kb.json"
        # parameters=_load_pcb_kb_items()
        print(parameters)
        async for chunk in self._call_llm_stream(
                system_prompt="格式化输出",
                user_prompt=XIMU_MNS_FLOW2_RANKING_BRIEF_PROMPT.format(previous_flow_context=instruction,parameters=parameters)):
    ...
    async def _call_llm(self, system_prompt: str, user_prompt: str) -> str:
        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30)) as s:
            async with s.post(f"{self.base_url}/chat/completions", json=payload, headers=headers) as resp:
                if resp.status == 200:
                    data = await resp.json()
                    return data["choices"][0]["message"]["content"]
                raise Exception(f"LLM call failed: {resp.status}, {await resp.text()}")
    ...
    async def _call_llm_stream(self, system_prompt: str, user_prompt: str):
        ...
        timeout = aiohttp.ClientTimeout(total=None)  # 流式不要 total=30
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(
                ...
                if resp.status != 200:
                    raise Exception(
                        f"LLM call failed: {resp.status}, {await resp.text()}"
                    )
