...
from src.material_pipeline import (
...
UPLOAD_DIR="upload"
app = FastAPI()
pipeline_service = MaterialPipelineService(repo_root=os.path.dirname(os.path.abspath(__file__)))
app.add_middleware(
    CORSMiddleware,
...
async def run_material_pipeline(req: PipelineRunRequest):
    accepted = await pipeline_service.submit(req)
    return accepted.model_dump()


@app.get("/pipeline/status/{task_id}")
async def get_material_pipeline_status(task_id: str):
    try:
...
async def get_material_pipeline_result(task_id: str):
    try:
        result = await pipeline_service.get_result(task_id)
