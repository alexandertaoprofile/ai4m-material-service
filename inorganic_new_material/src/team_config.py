...
# 软删说明：ADiT/MACE 历史流程暂不启用（保留代码结构，先不物理删除）
    ...
    async def _material_alignn_completion_stage(self, websocket, formula: str, llm=None):
        ...
        async def _stream_property_row(name: str, value, unit: str, source: str, hint: str, nd: int = 4):
            ...
            await websocket.send_text(
                f"| {name} | {value_text} | {unit} | {source} | {hint} |\n"
            )
    ...
    async def run(self, instruction: str, *args):
        ...
        def _extract_formulas_from_in_ls(repo_root: str) -> list:
            """
            ...
            try:
                ...
                if isinstance(st, dict):
                    for k in ("baseline_material", "advanced_material"):
                        v = st.get(k)
                        if isinstance(v, str) and v.strip():
                            tokens.append(_to_ascii_formula(v.strip()))
            except Exception as e:
        ...
        def _build_candidate_lists(raw_tokens: list):
            """
            分层处理候选：
        ...
        if True:
            ...
            in_ls_tokens = _extract_formulas_from_in_ls(_repo_root())
            if in_ls_tokens:
                # 合并第三来源，去重保持顺序
                raw_tokens = list(dict.fromkeys((raw_tokens or []) + in_ls_tokens))
            formulas, mp_formulas, non_mp_notes, dropped_tokens = _build_candidate_lists(raw_tokens)
