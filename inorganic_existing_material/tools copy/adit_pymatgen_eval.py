...
def main():
    ...
    # ----------------------------
    # 写 summary.md（横向 2×N，中文优先）
    # ----------------------------
    def _md_horizontal_table(title: str, kv: dict, max_cols: int = 12) -> list:
        out = []
        out.append(f"### {title}")
        out.append("")
        if not kv:
        ...
        keys = list(kv.keys())[:max_cols]
        labels = [_zh_key(str(k)) for k in keys]
        vals = [_fmt_value(kv.get(k)) for k in keys]

        out.append("| 指标 | " + " | ".join(labels) + " |")
        out.append("|---|" + "---:|" * len(labels))
        out.append("| 数值 | " + " | ".join(vals) + " |")
        out.append("")
        return out

    def _md_rule_table(kv: dict, title: str = "字段口径与关注点") -> list:
        out = []
        out.append(f"### {title}")
        out.append("")
        out.append("| 字段名 | 物理含义/关注点 |")
        out.append("|---|---|")
        for k in list(kv.keys())[:12]:
            out.append(f"| {_zh_key(str(k))} | {_zh_explain(str(k)) or '用于结构体检判读'} |")
        out.append("")
        return out
    ...
    lines.append("")
    lines.append(f"- 任务ID：`{taskid}`")
    lines.append(f"- 材料任务：`{jobid}`")
    lines.append(f"- 生成时间：`{datetime.utcnow().isoformat()}Z`")
    lines.append("")

    lines.append("## Pymatgen 快速体检")
    lines.append("")
    lines.extend(_md_horizontal_table("核心指标（横向）", pmg.get("metrics") or {}))
    lines.extend(_md_rule_table(pmg.get("metrics") or {}, title="字段口径与关注点（Pymatgen）"))
    lines.extend(_md_list_block("警告信息", pmg.get("warnings") or [], "⚠️"))
    lines.extend(_md_list_block("错误信息", pmg.get("errors") or [], "❌"))

    lines.append("## ADiT 合法性检查（轻量版本）")
    lines.append("")
    lines.extend(_md_horizontal_table("核心指标（横向）", adit.get("metrics") or {}))
    lines.extend(_md_rule_table(adit.get("metrics") or {}, title="字段口径与关注点（ADiT）"))

    pass_gate = bool((adit.get("metrics") or {}).get("pass_gate", False))
    gate_reason = str((adit.get("metrics") or {}).get("gate_reason", "")).strip()

    lines.append("### 准入判定（Gate）")
    lines.append("")
    lines.append("| 是否通过门槛 | 门槛原因 | 解释 |")
    lines.append("|---|---|---|")
    lines.append(
        f"| {'✅ 是' if pass_gate else '❌ 否'} | {gate_reason if gate_reason else '待计算'} | "
        f"{'当前结构通过基础合法性门槛，可继续后续计算。' if pass_gate else '当前结构未通过基础合法性门槛，建议先修复结构问题。'} |"
    )
    lines.append("")
    ...
    lines.extend(_md_list_block("警告信息", adit.get("warnings") or [], "⚠️"))
    lines.extend(_md_list_block("错误信息", adit.get("errors") or [], "❌"))
